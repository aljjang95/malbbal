(() => {
  ...
  const PANS = [
    ...
    },
    {
      ...
      no: "03",
      name: "가격깎기",
      tag: "깎는 게 아니라 선을 그는 일.",
      lines: [
        "이 가격이 최저예요. 더 깎으면 저희도 남는 게 없어요.",
        "다른 데 가보셔도 이 퀸리티는 이 값입니다.",
        "오늘 결정 안 하시면 이 조건은 사라져요."
      ],
      rewrites: {
        hedge: "오늘 계약하면 이 금액입니다. 그 위에서 움직이지 않습니다.",
        thin: "퀸리티는 인정합니다. 가격은 이 선에서 맞춰 주십시오.",
        fat: "최저라는 말에 올라타지 않습니다. 제가 지불할 숫자는 이미 정해 뤘습니다.",
  ...
  function coachLines(pan, scores, text) {
    ...
    if (scores.breath < 48 && !scores.typed) {
      ...
      lines.push("속도가 촛깁니다. 한 박자 낮추고 문장을 닫으세요.");
    }
    if (!text) {
    ...
    if (lines.length === 0) {
      if (lowest[1] >= 80) {
        lines.push("뼈는 셨습니다. 첫 문장만 더 차갑게.");
      } else if (lowest[0] === "칼집") {
